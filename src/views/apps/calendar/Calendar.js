...
const CalendarApp = () => {
  ...
  const htmlDescription = 'Full Calendar plugin.';

  const breadcrumbs = [
    { to: '', text: 'Home' },
    { to: 'apps', title: 'Calendar' },
  ];
  ...
  const onNewEventClick = () => {
    try {
      dispatch(setSelectedEvent({ id: 0, title: '', start: '', end: '' }));
  ...
  const handleDateSelect = (selectInfo) => {
    ...
    try {
      dispatch(setSelectedEvent({ id: 0, title: '', start: selectInfo.startStr, end: selectInfo.endStr }));
      setIsShowModalAddEdit(true);
    } catch (e) {
      console.log('This action could not be completed');
    }
  ...
  return (
    <>
        ...
        <Row className="g-0">
          ...
          <Col md="auto" className="d-flex align-items-start justify-content-end">
            <Button variant="outline-primary" className="btn-icon btn-icon-start ms-1 w-100 w-md-auto" onClick={onNewEventClick}>
              <CsLineIcons icon="plus" /> <span>Add Leave</span>
            </Button>
          </Col>
        </Row>
      </div>
      ...
      <section className="scroll-section" id="buttons">
          <h2 className="small-title mt-5">All Holidays</h2>
            <Row className="mb-5">
              <Col xl="12">
                <TabularDataBadges />
              </Col>
