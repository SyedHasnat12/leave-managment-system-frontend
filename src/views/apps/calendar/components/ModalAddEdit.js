...
const ModalAddEdit = ({ show = false, onHide = () => {} }) => {
  ...
  const categories = [
    { value: 'Annual Leave', label: 'Annual Leave' ,color: 'border-primary'  },
    { value: 'Sick Leave', label: 'Sick Leave'  ,  color: 'border-tertiary'},
    { value: 'Unpaid Leave', label: 'Unpaid Leave' , color: 'border-secondary'  },
    { value: 'Other', label: 'Other' ,  color: 'border-quaternary' },
  ];
  // const categories = [
  ...
  return (
    <>
      <Modal className="modal-right fade" show={show} onHide={onHide}>
        <Modal.Header>
          <Modal.Title>{selectedEvent.id !== 0 ? 'Edit Leave' : 'Add Leave'}</Modal.Title>
        </Modal.Header>
